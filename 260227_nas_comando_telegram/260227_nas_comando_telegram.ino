...
// ⚠️ Non usare ACCESO/SPENTO: collidono con NasState
#define LED_ON  LOW
#define LED_OFF HIGH
...
// -------- STATO NAS --------
enum class NasState : uint8_t { SPENTO, LAMPEGGIANTE, ACCESO };
enum class Pending  : uint8_t { NONE, TO_ACCESO, TO_SPENTO };

static NasState nasState = NasState::SPENTO;
static Pending  pending  = Pending::NONE;
static unsigned long transitionEndMs = 0;

static const unsigned long T_ACCENDI_MS = 4UL * 60UL * 1000UL; // 4 minuti
static const unsigned long T_SPEGNI_MS  = 3UL * 60UL * 1000UL; // 3 minuti

static String statoTextForCommand() {
  switch (nasState) {
    case NasState::LAMPEGGIANTE: return "nas lampeggiante";
    case NasState::ACCESO:       return "nas acceso";
    case NasState::SPENTO:       return "nas spento";
  }
  return "nas spento";
}
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    digitalWrite(LED_PIN, LED_OFF); delay(30);
    digitalWrite(LED_PIN, LED_ON);  delay(1000);
    prt(".");
  }
  digitalWrite(LED_PIN, LED_OFF);
  ...
  client.setInsecure();
  sendMsgImmediate(String(TELEGRAM_CHAT_ID), "Bot pronto (comandi: stato, accendi, spegni).");
  tgLastOkMs = millis();
}
