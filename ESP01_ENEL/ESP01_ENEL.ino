...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    hexString[sizeof(hexString) - 1] = '\0'; // Assicurati che la stringa sia terminata correttamente
    ...
    if (barray[0]== 1){
      ...
      // dati provenienti dal modulo ENEL
          // ricompone il valore uint16_t dai due bytes MSB (barray[2]) e LSB (barray[1])
      uint16_t contatoreWHA = (barray[2] << 8) | barray[1];

      //
      // // differenze con i valori della precedente
      // // ricezione via radio
      // float potenzaAttiva = 0;
      ...
      //   potenzaAttiva = 3600.0 / float(delta_tempo_sec) * float(delta_energia_wh);
      //   //potenzaAttivaL = long(potenzaAttiva);///100;
      //   //potenzaAttivaL = potenzaAttivaL*100;
